/**
 * @fileoverview Tests for awardFirstInterviewReward server action
 * @specification BLS-10-05 Wallet Stage - awardMilestoneReward (First Interview)
 *
 * Requirements tested:
 * - BLS-10-05.precondition.exists: Candidate must exist
 * - BLS-10-05.precondition.flag: Check isFirstInterviewerRewarded flag
...
describe('awardFirstInterviewReward', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    // Default mock implementations
    mockGet.mockResolvedValue({
      exists: true,
      data: () => ({ isFirstInterviewerRewarded: false }),
    });
    mockRunTransaction.mockImplementation(async (callback) => {
      const mockT = {
        get: vi.fn().mockResolvedValue({
          exists: true,
          data: () => ({ isFirstInterviewerRewarded: false, balance: 200 }),
        }),
        set: vi.fn(),
        update: vi.fn(),
      };
  ...
  describe('Input validation', () => {
    it('should return error for empty candidateId', async () => {
      const { awardFirstInterviewReward } = await import(
        '@/lib/database/actions/wallet-rewards'
      );

      const result = await awardFirstInterviewReward('');
  ...
  describe('Precondition: Candidate exists', () => {
    ...
    it('should return USER_NOT_FOUND if candidate does not exist', async () => {
      ...
      const result = await awardFirstInterviewReward('nonexistent-candidate');
  ...
  describe('Idempotency: No duplicate rewards', () => {
    ...
    it('should return alreadyAwarded=true if flag is already set', async () => {
      mockGet.mockResolvedValue({
        exists: true,
        data: () => ({ isFirstInterviewerRewarded: true }),
      });
      ...
      const result = await awardFirstInterviewReward('candidate-already-rewarded');
  ...
  describe('Success case: Award first interview bonus', () => {
    /**
     * Requirement: BLS-10-05.success
     * "Award 100 coins and set flag"
     */
    it('should successfully award first interview bonus', async () => {
      ...
      const result = await awardFirstInterviewReward('candidate-123');
    ...
    it('should run transaction to update balance and flag atomically', async () => {
      ...
      await awardFirstInterviewReward('candidate-456');
  ...
  describe('Error handling', () => {
    it('should return NETWORK_ERROR on database failure', async () => {
      ...
      const result = await awardFirstInterviewReward('candidate-error');
